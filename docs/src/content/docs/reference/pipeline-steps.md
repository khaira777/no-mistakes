...
- On GitHub, treats unresolved review-thread comments from supported review bots (Greptile, CodeRabbit, GitHub Code Quality/CodeQL, and Codex) as CI issues that block readiness, appear as structured findings at approval gates, and enter CI repair prompts when an auto-fix attempt starts; the comments are framed as untrusted external data and the rendered section is capped at 32 KiB
